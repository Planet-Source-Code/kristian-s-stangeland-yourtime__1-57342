...
Private mErrNumber As Long, mErrSource As String, mErrDescr As String, mErl As Long, mMethodHeader As String
Private mErrHelpFile As String, mErrHelpContext As String, mErrLastTime As Date
...
Public cNoRepeatErrors As New Collection
...
Public Sub ErrAddToList()

cNoRepeatErrors.Add mMethodHeader & "," & mErrNumber & "," & mErrAPI

End Sub

Public Function IsInList() As Boolean

On Error Resume Next
Dim sTmp, Vars

For Each sTmp In cNoRepeatErrors

    Vars = Split(sTmp, ",")
    
    If UBound(Vars) > 1 Then
        If mMethodHeader = Vars(0) And mErrNumber = Vars(1) And mErrAPI = Vars(2) Then
            IsInList = True
        End If
    End If
Next
...
Const TEMPLATE_REPORT = _
  "%Descr% %nl%" & _
  "  Time='%Time%' App='%App%:%Ver%' Computer='%Comp%' %nl%" & _
...
Public Sub HandleError()
    ...
    If InException Then
        ...
        End Select
    Else
        If Script.LogToEventLog Then ErrSaveToEventLog
        If Script.DumpError Then ErrSaveToFile
        If IsInList = False Then frmShowError.ErrorReport = ErrReport
        mErrLastTime = Now
    End If
